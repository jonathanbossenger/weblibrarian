...
class WEBLIB_Patron {
	...
	function __construct( $id = 0 ) {
		...
		if ( $this->theid == 0 ) {  /* Create a fresh Patron record */
			...
			$this->record = array(
				...
				'expiration'       => self::FiveYears(),
			);
		} elseif ( self::ValidPatronID( $this->theid ) ) {
			global $wpdb;
			/* Yank a record from the DB */
			$olderror  = $wpdb->show_errors( get_option( 'weblib_debugdb' ) != 'off' );
			$sql       = $wpdb->prepare(
				'SELECT firstname,lastname,extraname,address1,' .
	...
	function set_firstname( $fn ) {
		$this->record['firstname'] = $fn;
		$this->dirty               = true;}
	function lastname() {
		return $this->record['lastname']; }
	function set_lastname( $fn ) {
		$this->record['lastname'] = $fn;
		$this->dirty              = true;}
	function extraname() {
		return $this->record['extraname']; }
	function set_extraname( $fn ) {
		$this->record['extraname'] = $fn;
		$this->dirty               = true;}
	function address1() {
		return $this->record['address1']; }
	function set_address1( $ad ) {
		$this->record['address1'] = $ad;
		$this->dirty              = true;}
	function address2() {
		return $this->record['address2']; }
	function set_address2( $ad ) {
		$this->record['address2'] = $ad;
		$this->dirty              = true;}
	function city() {
	...
	function set_telephone( $t ) {
		$this->record['telephone'] = $t;
		$this->dirty               = true;}
	function outstandingfines() {
		return $this->record['outstandingfines']; }
	function set_outstandingfines( $f ) {
		$this->record['outstandingfines'] = $f;
		$this->dirty                      = true;}
	function expiration() {
	...
	function set_expiration( $e ) {
		$this->record['expiration'] = $e;
		$this->dirty                = true;}
	...
	function store( $newid = 0 ) {
		...
		if ( $this->record['firstname'] == '' ||
			...
			$this->record['expiration'] == '' ) {
			return( -1 );
		}
		if ( ! is_numeric( $this->record['outstandingfines'] ) ) {
			return( -1 );
		}
		global $wpdb;
		if ( $this->insert ) {
	...
	function delete() {
		...
		$this->dirty = false;
		self::DeletePatronByID( $this->theid );
	...
	static function AllPatrons() {
		...
		$patrons  = $wpdb->get_results(
			...
			WEBLIB_PATRONS . ' order by lastname,firstname',
			'ARRAY_A'
		);
		$wpdb->show_errors( $olderror );
		$result = array();
		foreach ( $patrons as $patron ) {
			...
			$result[] = array(
				'name'     => stripslashes( $patron['lastname'] ) . ', ' . stripslashes( $patron['firstname'] ) . ' ' . stripslashes( $patron['extraname'] ),
				'patronid' => $patron['id'],
			);
		}
	...
	static function DeletePatronByID( $id ) {
		...
		$olderror = $wpdb->show_errors( get_option( 'weblib_debugdb' ) != 'off' );
		$sql      = $wpdb->prepare(
			'DELETE FROM ' . WEBLIB_PATRONS . ' where id = %d',
			$id
		);
		$wpdb->query( $sql );
		$wpdb->show_errors( $olderror );
	...
	static function FindPatronByName( $pattern ) {
		...
		$sql      = $wpdb->prepare(
			...
			' like %s order by lastname, firstname',
			$pattern
		);
	...
	static function ValidPatronID( $patronid ) {
		...
		$olderror = $wpdb->show_errors( get_option( 'weblib_debugdb' ) != 'off' );
		$sql      = $wpdb->prepare(
			'SELECT count(id) FROM ' . WEBLIB_PATRONS .
			' where id = %d',
			$patronid
		);
	...
	static function PatronIdDropdown( $selected, $args, $echoit = true ) {
		...
		if ( $selected == 0 ) {
			...
			$result .= '></option>';
			$patrons = self::AllPatrons();
		foreach ( $patrons as $patron ) {
			if ( $onlyunassoc ) {
				$userid  = self::UserIDFromPatronID( $patron['patronid'] );
			...
			foreach ( $allcols as $col ) {
				switch ( $col ) {
					...
					case 'outstandingfines':
						...
						break;
					default:
						$csv .= $comma . '"' . self::csv_quote( stripslashes( $row[ $col ] ) ) . '"';
						break;
				}
				$comma = ',';
			}
...
class WEBLIB_ItemInCollection {
	...
	function __construct( $barcode = '' ) {
		...
		if ( $this->thebarcode != '' ) {
			global $wpdb;
			if ( self::IsItemInCollection( $this->thebarcode ) ) {
				$olderror  = $wpdb->show_errors( get_option( 'weblib_debugdb' ) != 'off' );
				$sql       = $wpdb->prepare(
					'SELECT * FROM ' . WEBLIB_COLLECTION .
					' where barcode = %s',
					$this->thebarcode
				);
				$rawrecord = $wpdb->get_row( $sql, 'ARRAY_A' );
	...
	function publocation() {
		return $this->record['publocation'];}
	...
	function set_pubdate( $s ) {
		$this->record['pubdate'] = $s;
		$this->dirty             = true;}
	function edition() {
		return $this->record['edition'];}
	...
	function delete() {
		...
		$this->dirty = false;
		self::DeleteItemByBarCode( $this->thebarcode );
		self::DeleteKeywordsByBarCode( $this->thebarcode );
	...
	function store( $newbarcode = '' ) {
		...
		if ( $this->insert ) {
			if ( $newbarcode != '' ) {
				...
				$wpdb->show_errors( $olderror );
				$newbarcode = self::incrstring( $lastbarcode );
				// file_put_contents("php://stderr","*** WEBLIB_ItemInCollection::store: newbarcode = '$newbarcode'\n");
				while ( self::IsItemInCollection( $newbarcode ) ) {
					$newbarcode = self::incrstring( $newbarcode );
					file_put_contents( 'php://stderr', "*** WEBLIB_ItemInCollection::store: newbarcode = '$newbarcode'\n" );
				}
				$this->thebarcode = $newbarcode;
	...
	static function FindItemByAuthor( $pattern, $orderby = 'barcode', $order = 'ASC' ) {
		...
		$sql         = $wpdb->prepare(
			'SELECT barcode FROM ' .
			WEBLIB_COLLECTION .
			' where author like %s order by ' . $orderby . ' ' . $order,
			$pattern
		);
		$rawbarcodes = $wpdb->get_col( $sql );
		$wpdb->show_errors( $olderror );
	...
	static function FindItemBySubject( $pattern, $orderby = 'barcode', $order = 'ASC' ) {
		...
		$sql         = $wpdb->prepare(
			'SELECT barcode FROM ' .
			WEBLIB_COLLECTION .
			' where subject like %s order by ' . $orderby . ' ' . $order,
			$pattern
		);
		$rawbarcodes = $wpdb->get_col( $sql );
		$wpdb->show_errors( $olderror );
	...
	static function FindItemByISBN( $pattern, $orderby = 'barcode', $order = 'ASC' ) {
		...
		$sql         = $wpdb->prepare(
			'SELECT barcode FROM ' .
			WEBLIB_COLLECTION .
			' where isbn like %s order by ' . $orderby . ' ' . $order,
			$pattern
		);
		$rawbarcodes = $wpdb->get_col( $sql );
		$wpdb->show_errors( $olderror );
	...
	function keywordsof() {
		...
		$olderror    = $wpdb->show_errors( get_option( 'weblib_debugdb' ) != 'off' );
		$sql         = $wpdb->prepare(
			'SELECT keyword FROM ' . WEBLIB_KEYWORDS .
			' where barcode = %s',
			$this->barcode()
		);
	...
	static function IsItemInCollection( $barcode ) {
		...
		$olderror = $wpdb->show_errors( get_option( 'weblib_debugdb' ) != 'off' );
		$sql      = $wpdb->prepare(
			'SELECT count(barcode) FROM ' . WEBLIB_COLLECTION .
			' where barcode = %s',
			$barcode
		);
	...
	static function upload_csv(
		...
		while ( $line = fgetcsv(
			...
			if ( $generatebarcode ) {
				$lastbarcode = self::incrstring( $lastbarcode );
				while ( self::IsItemInCollection( $lastbarcode ) ) {
					$lastbarcode = self::incrstring( $lastbarcode );
				}
				$data['barcode'] = $lastbarcode;
			} elseif ( ! preg_match( '/^[a-zA-Z0-9]+$/', $data['barcode'] ) ) {
					...
					$data['barcode'] = self::incrstring( $lastbarcode );
	...
	static function fixBrokenBarcodes() {
		...
		foreach ( $barcodes as $bc ) {
			if ( ! preg_match( '/^[a-zA-Z0-9]+$/', $bc ) ) {
				...
				$wpdb->show_errors( $olderror );
				$newbc     = self::incrstring( $lastbarcode );
...
class WEBLIB_OutItem {
	...
	function barcode() {
		return $this->record['barcode'];}
	function set_barcode( $v ) {
		$this->record['barcode'] = $v;
		$this->dirty             = true;}
	function title() {
		return $this->record['title'];}
	...
	function set_type( $v ) {
		...
		$this->dirty          = true;}
	function patronid() {
		return $this->record['patronid'];}
	function set_patronid( $v ) {
		$this->record['patronid'] = $v;
		$this->dirty              = true;}
	function dateout() {
		return $this->record['dateout'];}
	function set_dateout( $v ) {
		$this->record['dateout'] = $v;
		$this->dirty             = true;}
	function datedue() {
		return $this->record['datedue'];}
	function set_datedue( $v ) {
		$this->record['datedue'] = $v;
		$this->dirty             = true;}
	...
	function delete() {
		...
		$this->dirty = false;
		self::DeleteOutItemByTransaction( $this->thetransaction );
		$this->insert = true;
		return true;
	}
	function clean() {
	...
	static function RenewByBarcodeAndPatronID( $barcode, $patronid = '*' ) {
		...
		if ( $numberofholds > 0 ) {
			return sprintf(
				_n(
					'You cannot renew %1$s, it has %2$d hold.',
					'You cannot renew %1$s, it has %2$d holds.',
					$numberofholds,
					'weblibrarian'
				),
				$outitem->title(),
		...
		if ( $renewals > 3 ) {
			return sprintf(
				__(
					'Maximum number of renewals reached for %s!',
					'weblibrarian'
				),
				$outitem->title()
			);
	...
	static function RemoveOrphanCheckouts() {
		...
		foreach ( $transactions as $trans ) {
			...
			if ( ! WEBLIB_ItemInCollection::IsItemInCollection( $out->barcode() ) ) {
				...
				$out->delete();
			} elseif ( ! WEBLIB_Patron::ValidPatronID( $out->patronid() ) ) {
				$orphans[] = array(
					'transaction' => $trans,
					'barcode'     => $out->barcode(),
					'patronid'    => $out->patronid(),
	...
	static function OutItemByBarcode( $barcode ) {
		...
		if ( empty( $transactions ) ) {
			...
			return new WEBLIB_OutItem( $transactions[0] );
		}
	}
...
class WEBLIB_HoldItem {
	...
	function set_patronid( $v ) {
		...
		$this->dirty              = true;}
	function dateheld() {
		return $this->record['dateheld'];}
	function set_dateheld( $v ) {
		$this->record['dateheld'] = $v;
		$this->dirty              = true;}
	function dateexpire() {
		return $this->record['dateexpire'];}
	function set_dateexpire( $v ) {
		$this->record['dateexpire'] = $v;
		$this->dirty                = true;}
	function store( $newtransaction = 0 ) {
	...
	function delete() {
		...
		$this->dirty = false;
		self::DeleteHeldItemByTransaction( $this->thetransaction );
		$this->insert = true;
		return true;
	}
	function clean() {
	...
	static function RemoveOrphanHolds() {
		global $wpdb;
		$transactions = self::AllHeldItems();
		$orphans      = array();
		foreach ( $transactions as $trans ) {
			...
			if ( ! WEBLIB_ItemInCollection::IsItemInCollection( $hold->barcode() ) ) {
				...
				$hold->delete();
			} elseif ( ! WEBLIB_Patron::ValidPatronID( $hold->patronid() ) ) {
				$orphans[] = array(
					'transaction' => $trans,
					'barcode'     => $hold->barcode(),
					'patronid'    => $hold->patronid(),
...
class WEBLIB_Statistic {
	...
	function __construct(
		...
		$olderror       = $wpdb->show_errors( get_option( 'weblib_debugdb' ) != 'off' );
		$sql            = $wpdb->prepare(
			'SELECT count FROM ' . WEBLIB_STATISICS .
	...
	static function MonthTotal( $year, $month ) {
		...
		$olderror = $wpdb->show_errors( get_option( 'weblib_debugdb' ) != 'off' );
		$sql      = $wpdb->prepare(
			'SELECT sum(count) FROM ' . WEBLIB_STATISICS .
...
class WEBLIB_Type {
	...
	function __construct( $type ) {
		...
		$olderror = $wpdb->show_errors( get_option( 'weblib_debugdb' ) != 'off' );
		$sql      = $wpdb->prepare(
			'SELECT loanperiod FROM ' . WEBLIB_TYPES .
				' WHERE type = %s',
			$this->thetype
		);
	...
	static function KnownType( $type ) {
		...
		$olderror = $wpdb->show_errors( get_option( 'weblib_debugdb' ) != 'off' );
		$sql      = $wpdb->prepare(
			'SELECT count(type) FROM ' . WEBLIB_TYPES .
			' WHERE type = %s',
			$type
		);
		$result   = ( $wpdb->get_var( $sql ) > 0 );

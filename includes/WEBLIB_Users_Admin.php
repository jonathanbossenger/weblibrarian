...
class WEBLIB_Users_Admin extends WP_List_Table {
	...
	function __construct( $pa_pages ) {
		...
		$screen_id = add_submenu_page(
			'users.php',
			__( 'Edit your Patron info', 'weblibrarian' ),
			__( 'Edit Patron info', 'weblibrarian' ),
			'read',
			'weblib-edit-your-patron-info',
		...
		$screen_id = add_submenu_page(
			'users.php',
			__( 'Add Patron ID to a user', 'weblibrarian' ),
			__( 'Add Patron ID', 'weblibrarian' ),
			'edit_users',
			'weblib-add-patron-id-to-a-user',
			array( $this, 'add_patron_id' )
		);
	...
	*/
	...
	function prepare_items() {
		...
		if ( isset( $_REQUEST['setid'] ) ) {
			...
			if ( ! $patron->StorePatronIDWithSelectedUser( $user_id, $error, true ) ) {
				...
				$message = '<p>' . sprintf(
					__(
						'Patron ID %1$d (%2$s) has been set for user %3$d (%4$s).',
						'weblibrarian'
					),
					$patronid,
					WEBLIB_Patron::NameFromId( $patronid ),
					$user_id,
					$user->user_login
				) . '</p>';
			}
	...
	function edit_your_patron_info() {
		...
		?>
	<div class="wrap"><div id="icon-users" class="icon32"><br /></div><h2><?php _e( 'Edit your patron info', 'weblibrarian' ); ?></h2>
		<?php
		if ( $message != '' ) {
			?>
	...
	<input type="hidden" name="page" value="weblib-edit-your-patron-info" />
		<?php
		switch ( $formtype ) {
			case 'none':
				break;
			case 'edit':
				$this->patron_admin_pages->display_one_item_form( null );
				break;
			case 'setid':
				WEBLIB_Patron::PatronIdDropdown( $patronid, array( 'onlyunassoc' => true ) );
				?>
	<input type="submit" value="<?php _e( 'Set Your Patron Id', 'weblibrarian' ); ?>" />
				<?php
				break;
		}
		?>
	...
	<h2><?php _e( 'Add Patron Ids to Users', 'weblibrarian' ); ?></h2>
															<?php
															if ( $message != '' ) {
																?>
		<div id="message" class="update fade"><?php echo $message; ?></div>

...
class WEBLIB_Statistics_Admin extends WP_List_Table {
	...
	function __construct() {
		...
		$screen_id = add_menu_page(
			__( 'Circulation Statistics', 'weblibrarian' ),
			__( 'Circulation Stats', 'weblibrarian' ),
			'manage_circulation',
		...
		$screen_id = add_submenu_page(
			'weblib-circulation-statistics',
			__( 'Export Circulation Stats', 'weblibrarian' ),
			__( 'Export', 'weblibrarian' ),
			'manage_circulation',
	...
	function get_columns() {
		if ( $this->mode == 'monthtotal' ) {
			return array(
				'label' => __( 'Month', 'weblibrarian' ),
				'value' => __( 'Count', 'weblibrarian' ),
			);
		} else {
			return array(
				'label' => __( 'Type', 'weblibrarian' ),
				'value' => __( 'Count', 'weblibrarian' ),
			);
		}
	}
	...
	<input type="hidden" name="month" value="<?php echo esc_attr( $this->month ); ?>" />
														<?php
		}
	...
	<select id="year_<?php echo $which; ?>" name="year_<?php echo $which; ?>">
								<?php
	...
	<select id="month_<?php echo $which; ?>" name="month_<?php echo $which; ?>">
								<?php
	...
	function prepare_items() {
		...
		} elseif ( isset( $_REQUEST['filter_bottom'] ) ) {
	...
	<div class="wrap"><div id="icon-statistics" class="icon32"><br /></div>
		<h2><?php _e( 'Library Circulation Statistics', 'weblibrarian' ); ?> <a href="
																	<?php
																		echo add_query_arg(
																			array( 'page' => 'weblib-export-circulation-statistics' )
																		);
																	?>
	" class="button add-new-h2"><?php _e( 'Export Stats', 'weblibrarian' ); ?></a></h2>
		<?php
		if ( $message != '' ) {
			?>
	...
	<div class="wrap"><div id="icon-export-statistics" class="icon32"><br /></div>
		...
		$month = date( 'm', time() );
		?>
		<input type="hidden" name="action" value="ExportLibraryStats" /><p><label for="year"><?php _e( 'Year', 'weblibrarian' ); ?></label><select id="year" name="year">
		<?php
		$allyears = WEBLIB_Statistic::AllYears();
		if ( empty( $allyears ) ) {
			$allyears[] = $year;}
	...
	<p><label for="month"><?php _e( 'Month', 'weblibrarian' ); ?></label><select id="month" name="month">
															<?php
															foreach ( $this->MonthNames as $m => $mtext ) {
																?>
		<option value="<?php echo $m; ?>"

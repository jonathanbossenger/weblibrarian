		...
		switch ( $action ) {
			case 'removehold':
				if ( isset( $_REQUEST['checked'] ) && ! empty( sanitize_text_field( $_REQUEST['checked'] ) ) ) {
					...
					WEBLIB_HoldItem::DeleteHeldItemByBarcodeAndPatronId(
						sanitize_text_field( $_REQUEST['barcode'] ),
						$this->patronid
					);
				}
				break;
...
class WEBLIB_PatronOutRecord_Admin extends WEBLIB_PatronRecord_Common {
	...
	function patron_outs() {
		...
		?>
	<div class="wrap"><div id="icon-users" class="icon32"><br /></div>
		<h2><?php echo __( 'Your Checked out Items', 'weblibrarian' ); ?></h2>
					<?php
					if ( $message != '' ) {
						?>
	<div id="message" class="update fade"><?php echo $message; ?></div>
	...
	function process_bulk_action() {
		...
		} elseif ( isset( $_REQUEST['action2'] ) && sanitize_text_field( $_REQUEST['action2'] ) != -1 ) {

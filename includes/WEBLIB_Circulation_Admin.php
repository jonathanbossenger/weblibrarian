...
require_once __DIR__ . '/WEBLIB_Collection_Admin.php';
...
class WEBLIB_Circulation_Admin extends WEBLIB_Collection_Shared {
	...
	function __construct( $args = array() ) {
		...
		$screen_id      = add_menu_page(
			__( 'Circulation Desk', 'weblibrarian' ),
			__( 'Circulation Desk', 'weblibrarian' ),
			'manage_circulation',
			'weblib-circulation-desk',
	...
	function extra_tablenav( $which ) {
		...
		if ( $this->mode == 'checkinpage' ) {
			foreach ( $this->checkinlist as $index => $bc ) {
				?>
	<input type="hidden" name="checkinlist[<?php echo $index; ?>]" value="<?php echo stripslashes( $bc ); ?>" />
				<?php
			}
		}
	...
	<div class="circulation-desk"><div class="weblib-row"><div id="weblib-inputs"><div class="weblib-inputitem"><label for="barcode" class="inputlab"><?php _e( 'Scanned Barcode', 'weblibrarian' ); ?></label><input id="barcode" name="barcode" value="<?php echo stripslashes( $this->barcode ); ?>" class="weblib-input-fill" />
																																																	<?php
																																																	switch ( $this->mode ) {
																																																		case 'checkinpage':
																																																			?>
	<input class="button" type="submit" name="checkinitem" value="<?php _e( 'Check in Item', 'weblibrarian' ); ?>" />
																																																			<?php
																																																			break;
																																																		case 'patroncircrecord':
																																																			?>
	<input class="button" type="submit" name="checkoutitem" value="<?php _e( 'Checkout Item', 'weblibrarian' ); ?>" />
																																																			<?php
																																																			break;
																																																		default:
																																																			?>
	<input class="button" type="submit" name="barcodelookup" value="<?php _e( 'Lookup Barcode', 'weblibrarian' ); ?>" />
																																																			<?php
																																																			break;
																																																	}
																																																	?>
	...
	<div class="weblib-inputitem">
			<?php
			WEBLIB_Patron::PatronIdDropdown(
				$this->patronid,
				array(
					'selectclass' => 'weblib-input-fill',
					'labelclass'  => 'inputlab',
				)
			);
			?>
	...
	<div class="weblib-inputitem-button"><input class="button" type="submit" name="listouts" value="<?php _e( 'List Checked Out Items', 'weblibrarian' ); ?>" /></div>
																																						<?php
		}
		if ( $this->mode != 'circulationdesk' ) {
			?>
		<div class="weblib-inputitem-button"><input class="button" type="submit" name="resetmode" value="<?php _e( 'Back to Main Circulation', 'weblibrarian' ); ?>" /></div>
			<?php
		}
		?>
...
<p class="search-box">
	...
	function prepare_items() {
		...
		} elseif ( isset( $_REQUEST['patronlookup'] ) && $this->mode != 'checkinpage' ) {
			$this->mode = 'patroncircrecord';
			/*
			unset($this->_column_headers);
			$this->get_column_info();*/
		} elseif ( $this->mode == 'patroncircrecord' &&
		isset( $_REQUEST['checkoutitem'] ) ) {
			...
			if ( $outitem != null ) {
				...
				if ( $checkouttrans > 0 ) {
					...
					$message     .= sprintf(
						__(
							'Item checked out, transaction is %1$d, due: %2$s.',
							'weblibrarian'
						),
						$checkouttrans,
					...
					$message .= sprintf(
						__(
							'Error checking out!  Result code is %d.',
							'weblibrarian'
						),
						$checkouttrans
					);
		...
		if ( $pagenum < 1 ) {
			$pagenum = 1;
		} elseif ( $pagenum > $total_pages && $total_pages > 0 ) {
			$pagenum = $total_pages;
		}
	...
	<h2>
		...
		switch ( $this->mode ) {
			case 'circulationdesk':
				_e( 'Library Circulation Desk', 'weblibrarian' );
				break;
			case 'checkinpage':
				_e( 'Library Circulation Desk -- Check Items In', 'weblibrarian' );
				break;
			case 'holdlist':
				_e( 'Library Circulation Desk -- Items with Holds', 'weblibrarian' );
				break;
			case 'outlist':
				_e( 'Library Circulation Desk -- Items Checked out', 'weblibrarian' );
				break;
			case 'patroncircrecord':
				printf( __( "Library Circulation Desk -- %s's Circulation Record", 'weblibrarian' ), WEBLIB_Patron::NameFromId( $this->patronid ) );
				break;
			case 'itemcircrecord':
				printf( __( 'Library Circulation Desk -- Circulation Record for %s', 'weblibrarian' ), $this->barcode );
				break;
			default:
				break;
		}

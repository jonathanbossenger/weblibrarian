...
class WEBLIB_ShortCodes {
	...
	function search_form( $atts, $content = null, $code = '' ) {
		...
		foreach ( array(
			'barcode' => __( 'System Sorted', 'weblibrarian' ),
			'title'   => __( 'Title', 'weblibrarian' ),
			'author'  => __( 'Author', 'weblibrarian' ),
		) as $field => $label ) {
			$result .= '<option value="' . $field . '"';
			if ( $field == $weblib_orderby ) {
				$result .= ' selected="selected"';}
			$result .= '>' . $label . "</option>\n";
		}
		$result .= "</select>\n";
		...
		foreach ( array(
			'ASC'  => __( 'Ascending', 'weblibrarian' ),
			'DESC' => __( 'Descending', 'weblibrarian' ),
		) as
		$value => $label ) {
			$result .= '<option value="' . $value . '"';
			if ( $value == $weblib_order ) {
				$result .= ' selected="selected"';}
			$result .= '>' . $label . "</option>\n";
		}
		$result .= "</select>\n";
	...
	function item_list( $atts, $content = null, $code = '' ) {
		...
		if ( $inlinemoreinfo ) {
			...
			if ( isset( $_REQUEST['barcode'] ) ) {
				return $result . $this->item_detail(
					array(
		...
		$result .= $this->generate_pagination(
			$pagenum,
			$total_pages,
			$per_page,
			array(
	...
	function item_detail( $atts, $content = null, $code = '' ) {
		...
		switch ( $detaillevel ) {
			case 'long':
				...
				if ( $numberofholds > 0 ) {
					$result .= sprintf(
						_n( '%d Hold', '%d Holds', $numberofholds, 'weblibrarian' ),
						$numberofholds
					);
				}

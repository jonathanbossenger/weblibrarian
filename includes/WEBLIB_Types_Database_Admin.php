...
class WEBLIB_Types_Database_Admin extends WP_List_Table {
	...
	function __construct() {
		...
		$screen_id = add_menu_page(
			__( 'Circulation Types Database', 'weblibrarian' ),
			__( 'Circulation Types', 'weblibrarian' ),
			'manage_collection',
		...
		$screen_id = add_submenu_page(
			'weblib-item-types-database',
			__( 'Add New Item Type', 'weblibrarian' ),
	...
	<h2><?php _e( 'Circulation Types', 'weblibrarian' ); ?> <a href="
														<?php
														echo add_query_arg(
															array(
																'page'                         => 'weblib-add-item-type',
	...
	<input type="hidden" name="page" value="weblib-add-item-type" />
		...
		$this->display_one_item_form(
			add_query_arg(
				array(
					'page' => 'weblib-item-types-database',
					'mode' => false,
					'id'   => false,
				)
			)
	...
	function set_row_actions( $racts ) {
		$this->row_actions = $racts; }
	...
	function add_item_h2() {
		switch ( $this->viewmode ) {
			case 'edit':
				return __( "Edit A Circulation Type's Loan Period", 'weblibrarian' );
			default:
			case 'add':
				return __( 'Add new Circulation Type', 'weblibrarian' );
		}
	}
	...
	function prepare_one_item() {
		...
		if ( isset( $_REQUEST['addtype'] ) ) {
			...
			if ( $message == '' ) {
				...
				$item->store();
				$message              = '<p>' . sprintf(
					__( '%1$s inserted, with a loan period of %2$d.', 'weblibrarian' ),
					$item->type(),
					$item->loanperiod()
				) . '</p>';
				$this->viewmode       = 'edit';
		...
		} elseif ( isset( $_REQUEST['updatetype'] ) ) {
			$this->viewmode = 'edit';
			$message        = $this->checkiteminform();
			$this->getitemfromform();
			if ( $message == '' ) {
				...
				$item->store();
				$message = '<p>' . sprintf(
					__( '%1$s updated, with a loan period of %2$d.', 'weblibrarian' ),
					$item->type(),
					$item->loanperiod()
				) . '</p>';
			}
			...
			switch ( $this->viewmode ) {
				case 'edit':
					...
					} elseif ( WEBLIB_Type::KnownType( $this->viewtypename ) ) {
						$item                 = new WEBLIB_Type( $this->viewtypename );
						$this->viewloanperiod = $item->loanperiod();
					} else {
						$this->viewmode       = 'add';
	...
	<td><input id="typename"
			...
			style="width:75%;"
			maxlength="16"
			value="<?php echo stripslashes( $this->viewtypename ); ?>"
							<?php
							if ( $this->viewmode != 'add' ) {
	...
	<td><input id="loanperiod"
		...
		switch ( $this->viewmode ) {
			case 'add':
				?>
			<input type="submit" name="addtype" class="button-primary" value="<?php _e( 'Add New Type', 'weblibrarian' ); ?>" />
				<?php
				break;
			case 'edit':
				?>
			<input type="submit" name="updatetype" class="button-primary" value="<?php _e( 'Update Type', 'weblibrarian' ); ?>" />
				<?php
				break;
		}
		?>

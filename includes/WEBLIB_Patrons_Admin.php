...
class WEBLIB_Patrons_Admin extends WP_List_Table {
	...
	function __construct() {
		...
		$screen_id = add_menu_page(
			__( 'Library Patrons', 'weblibrarian' ),
			__( 'Patrons', 'weblibrarian' ),
			'manage_patrons',
			'weblib-list-patrons',
			array( $this, 'list_patrons' ),
		...
		$screen_id = add_submenu_page(
			'weblib-list-patrons',
			__( 'Add Library Patron', 'weblibrarian' ),
			__( 'Add New', 'weblibrarian' ),
			'manage_patrons',
			'weblib-add-patron',
			array( $this, 'add_patron' )
		);
		$weblib_contextual_help->add_contextual_help( $screen_id, 'weblib-add-patron' );
		$screen_id = add_submenu_page(
			'weblib-list-patrons',
			__( 'Add Bulk Library Patrons', 'weblibrarian' ),
	...
	*/
	...
	function column_telephone( $item ) {
		return self::addtelephonedashes( WEBLIB_Patron::TelephoneFromId( $item['patronid'] ) );
	}
	...
	function process_bulk_action() {
		...
		switch ( $action ) {
			case 'delete':
				...
				} elseif ( isset( $_REQUEST['patronid'] ) ) {
	...
	<h2><?php _e( 'Library Patrons', 'weblibrarian' ); ?> <a href="
													<?php
													echo add_query_arg(
														array(
															'page' => 'weblib-add-patron',
	...
	" class="button add-new-h2">
		<?php
		_e( 'Add New', 'weblibrarian' );
		?>
	</a> <a href="
		<?php
		echo add_query_arg(
			array( 'page' => 'weblib-add-patron-bulk' ),
			admin_url( 'admin.php' )
		);
		?>
	" class="button add-new-h2">
	...
	function prepare_one_item( $args = array() ) {
		...
		if ( isset( $_REQUEST['addpatron'] ) ) {
			...
			if ( $message == '' ) {
				...
				if ( $newid < 0 ) {
					...
					$message        = '<p>' . sprintf(
						__( '%1$s, %2$s %3$s inserted with Patron Id %4$d.', 'weblibrarian' ),
						$item->lastname(),
						$item->firstname(),
						$item->extraname(),
						$newid
		...
		} elseif ( isset( $_REQUEST['updatepatron'] ) &&
		isset( $_REQUEST['patronid'] ) ) {
			...
			if ( $message == '' ) {
				$item->store();
				$message = '<p>' . sprintf(
					__( '%1$s, %2$s %3$s updated.', 'weblibrarian' ),
					$item->lastname(),
					$item->firstname(),
					$item->extraname()
				) . '</p>';
	...
	function checkiteminform( $id ) {
		$result       = '';
		$newtelephone = self::striptelephonedashes( $_REQUEST['telephone'] );
		...
		self::ValidHumanDate( $newexpiration, $theexpiration, __( 'Expiration', 'weblibrarian' ), $result );
		return $result;
	}
	function getitemfromform( $id ) {
		$patron = new WEBLIB_Patron( $id );
		$patron->set_telephone( self::striptelephonedashes( $_REQUEST['telephone'] ) );
		...
		$dummy = '';
		self::ValidHumanDate( $_REQUEST['expiration'], $theexpiration, __( 'Expiration', 'weblibrarian' ), $dummy );
	...
	function add_item_icon() {
		switch ( $this->viewmode ) {
			case 'edit':
				return 'icon-patron-edit';
			case 'view':
				return 'icon-patron-view';
			default:
			case 'add':
				return 'icon-patron-add';
		}
	}
	function add_item_h2() {
		switch ( $this->viewmode ) {
			case 'edit':
				return __( 'Edit Patron Info', 'weblibrarian' );
			case 'view':
				return __( 'View Patron Info', 'weblibrarian' );
			default:
			case 'add':
				return __( 'Add new Patron', 'weblibrarian' );
		}
	}

	static function ValidHumanDate( $datestring, &$mysqldate, $label, &$error ) {
		$Months = array(
			__( 'jan', 'weblibrarian' ) => 1,
		...
		if ( $dfmt == '%m/%d/%y' ) {
			$invdate = __( 'Invalid %1$s date (%2$s). Should be mm/yyyy or mm/dd/yyyy.', 'weblibrarian' );
		} else {
			$invdate = __( 'Invalid %1$s date (%2$s). Should be mm/yyyy or dd/mm/yyyy.', 'weblibrarian' );
		}
		...
		if ( ! is_int( $month ) ) {
			...
			if ( strlen( $lowmonth ) > 3 ) {
				$lowmonth = substr( $lowmonth, 0, 3 );}
			if ( isset( $Months[ $lowmonth ] ) ) {
				...
				$error .= sprintf( __( 'Invalid %1$s date (%2$s): illegal month (%3$s). Should be one of %4$s.', 'weblibrarian' ), $label, $datestring, $month, $mlist );
				$error .= '</span>';
				return false;
			}
		}
		if ( ! checkdate( $month, $date, $year ) ) {
			$error .= '<br /><span id="error">';
			$error .= sprintf( __( 'Invalid %1$s date (%2$s). Out of range.', 'weblibrarian' ), $label, $datestring );
			$error .= '</span>';
			return false;
		}
	...
	function display_one_item_form( $returnURL ) {
		...
		if ( isset( $_REQUEST['screen-options-apply'] ) ) {
			...
			?>
		<input type="hidden" name="screen-options-apply" value="<?php echo $screenopts; ?>" />
			<?php
		}
		if ( isset( $_REQUEST['wp_screen_options']['option'] ) ) {
			$wpscreenoptsopt = sanitize_text_field( $_REQUEST['wp_screen_options']['option'] );
			?>
		<input type="hidden" name="wp_screen_options[option]" value="<?php echo $wpscreenoptsopt; ?>" />
			<?php
		}
		if ( isset( $_REQUEST['wp_screen_options']['value'] ) ) {
			$wpscreenoptsval = sanitize_text_field( $_REQUEST['wp_screen_options']['value'] );
			?>
		<input type="hidden" name="wp_screen_options[value]" value="<?php echo $wpscreenoptsval; ?>" />
			<?php
		}
	...
	<td><input id="telephone"
			...
			value="
			<?php
			echo self::addtelephonedashes( $this->viewitem->telephone() );
			?>
		"<?php echo $ro; ?> /></td></tr>
		<tr valign="top">
	...
	<td><input id="expiration"
		...
		switch ( $this->viewmode ) {
			case 'add':
				?>
			<input type="submit" name="addpatron" class="button-primary" value="<?php _e( 'Add Patron', 'weblibrarian' ); ?>" />
				<?php
				break;
			case 'edit':
				?>
			<input type="submit" name="updatepatron" class="button-primary" value="<?php _e( 'Update Patron', 'weblibrarian' ); ?>" />
				<?php
				break;
		}
		if ( $returnURL != '' ) {
	...
	function process_bulk_upload() {
		...
		$result = WEBLIB_Patron::upload_csv(
			$filename,
			$use_csv_headers,
			$field_sep,

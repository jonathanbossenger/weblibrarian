...
class WEBLIB_Collection_Admin extends WEBLIB_Collection_Shared {
	...
	function __construct( $args = array() ) {
		...
		$screen_id      = add_menu_page(
			__( 'Collection Database', 'weblibrarian' ),
			__( 'Collection', 'weblibrarian' ),
			'manage_collection',
			'weblib-collection-database',
		...
		$screen_id = add_submenu_page(
			'weblib-collection-database',
			__( 'Add Item to Collection', 'weblibrarian' ),
			__( 'Add New', 'weblibrarian' ),
			'manage_collection',
			'weblib-add-item-collection',
			array( $this, 'add_item' )
		);
		$weblib_contextual_help->add_contextual_help( $screen_id, 'weblib-add-item-collection' );
		$screen_id = add_submenu_page(
			'weblib-collection-database',
			__( 'Add Bulk Items to Collection', 'weblibrarian' ),
			__( 'Add New Bulk', 'weblibrarian' ),
			'manage_collection',
			'weblib-add-item-collection-bulk',
			array( $this, 'add_item_bulk' )
		);
		$weblib_contextual_help->add_contextual_help( $screen_id, 'weblib-add-item-collection-bulk' );
		$screen_id = add_submenu_page(
			'weblib-collection-database',
			__( 'DB Maintenance', 'weblibrarian' ),
			__( 'DB Maint', 'weblibrarian' ),
			'manage_collection',
	...
	function column_title( $item ) {
		...
		$actions = array(
			'edit'   => '<a href="' . add_query_arg(
				array(
					'page'    => 'weblib-add-item-collection',
					'mode'    => 'edit',
					'barcode' => $item,
				),
				admin_url( 'admin.php' )
	...
	<h2><?php _e( 'Library Collection', 'weblibrarian' ); ?> <a href="
														<?php
														echo add_query_arg(
															array(
																'page' => 'weblib-add-item-collection',
														...
														?>
	" class="button add-new-h2"><?php _e( 'Add New', 'weblibrarian' ); ?></a> <a href="
		<?php
		echo add_query_arg( array( 'page' => 'weblib-add-item-collection-bulk' ) );
		?>
	" class="button add-new-h2">
		<?php
		_e( 'Add New Bulk', 'weblibrarian' );
		?>
	</a> <a href="
		<?php
			echo add_query_arg(
				array(
					'dataselection' => 'collection',
	...
	" class="button add-new-h2">
		<?php
		_e( 'Export as CSV', 'weblibrarian' );
		?>
	</a></h2>
		<?php
		if ( $message != '' ) {
			?>
		<div id="message" class="update fade"><?php echo $message; ?></div>
			<?php
		}
		?>
	...
	<input type="hidden" name="page" value="weblib-collection-database" />
		...
		<?php
		submit_button(
			__( 'Fix Broken Barcodes', 'weblibrarian' ),
			'secondary',
			'fixbrokenbarcodes',
			false,
	...
	<input type="hidden" name="page" value="weblib-add-item-collection" />
		<?php
		$this->display_one_item_form(
			add_query_arg(
				array(
					'page'    => 'weblib-collection-database',
					'mode'    => false,
					'barcode' => false,
				)
			)
	...
	function prepare_one_item() {
		...
		if ( isset( $_REQUEST['additem'] ) ) {
			...
			if ( $message == '' ) {
				...
				$message            = '<p>' . sprintf(
					__( '%1$s inserted with barcode %2$s.', 'weblibrarian' ),
					$item->title(),
					$newbarcode
				) . '</p>';
				$this->viewmode     = 'edit';
		...
		} elseif ( isset( $_REQUEST['updateitem'] ) &&
	...
	function add_item_icon() {
		switch ( $this->viewmode ) {
			case 'view':
				return 'icon-item-view';
			case 'edit':
				return 'icon-item-edit';
			default:
			case 'add':
				return 'icon-item-add';
		}
	}
	function add_item_h2() {
		switch ( $this->viewmode ) {
			case 'view':
				return __( 'View an item in the collection', 'weblibrarian' );
			case 'edit':
				return __( 'Edit an item in the collection', 'weblibrarian' );
			default:
			case 'add':
				return __( 'Add a new item to the collection', 'weblibrarian' );
		}
	}
	...
	function display_one_item_form( $returnURL ) {
		...
		if ( isset( $_REQUEST['screen-options-apply'] ) ) {
			...
			?>
		<input type="hidden" name="screen-options-apply" value="<?php echo esc_attr( $screenopts ); ?>" />
			<?php
		}
		if ( isset( $_REQUEST['wp_screen_options']['option'] ) ) {
			$wpscreenoptsopt = sanitize_text_field( $_REQUEST['wp_screen_options']['option'] );
			?>
		<input type="hidden" name="wp_screen_options[option]" value="<?php echo esc_attr( $wpscreenoptsopt ); ?>" />
			<?php
		}
		if ( isset( $_REQUEST['wp_screen_options']['value'] ) ) {
			$wpscreenoptsval = sanitize_text_field( $_REQUEST['wp_screen_options']['value'] );
			?>
		<input type="hidden" name="wp_screen_options[value]" value="<?php echo esc_attr( $wpscreenoptsval ); ?>" />
			<?php
		}
	...
	<td><input id="barcode"
					...
					style="width:75%;"
					maxlength="16"
					value="<?php echo esc_attr( stripslashes( $this->viewbarcode ) ); ?>"
									<?php
	...
	<td>
		...
		<input id="type" 
			...
			?>
		<select id="type" name="type" style="width:75%;">
			<?php
			$alltypes     = WEBLIB_Type::AllTypes();
			$existingtype = $this->viewitem->type();
			if ( $existingtype == '' ) {
				$existingtype = $alltypes[0];
			}
			foreach ( $alltypes as $atype ) {
				?>
			<option value="<?php echo esc_attr( $atype ); ?>"
				<?php
				if ( $atype == $existingtype ) {
					echo ' selected="selected"';
				}
				?>
			><?php echo esc_html( $atype ); ?></option>
				<?php
			}
			?>
		</select>
	...
	<div id="itemedit-keyword-div">
		...
		<div class="jaxkeyword">
			...
			<p>Add or remove keywords</p>
				<?php
		} else {
			?>
		<label for="itemedit-keyword-list"><?php _e( 'Keywords:', 'weblibrarian' ); ?></label><br />
			<?php
		}
		?>
		<textarea id="itemedit-keyword-list" name="keywordlist" 
				rows="3" cols="20" class="the-keywords"<?php echo $ro; ?> >
																	<?php
																	echo esc_html( implode( ',', $this->viewkeywords ) );
																	?>
																	</textarea>
			<?php
			if ( $this->viewmode != 'view' ) {
		...
		<script type="text/javascript">
			WEBLIB_WriteKeywords('itemedit');</script></div>
				<?php
			}
			?>
			</div></td></tr>
		...
		switch ( $this->viewmode ) {
			case 'add':
				?>
			<input type="submit" name="additem" class="button-primary" value="<?php _e( 'Add New Item', 'weblibrarian' ); ?>" />
				<?php
				break;
			case 'edit':
				?>
			<input type="submit" name="updateitem" class="button-primary" value="<?php _e( 'Update Item', 'weblibrarian' ); ?>" />
				<?php
				break;
		}
		?>
	...
	function process_bulk_upload() {
		...
		$result = WEBLIB_ItemInCollection::upload_csv(
			$filename,
			$use_csv_headers,
			$field_sep,
			$enclose_char
	...
	function display_bulk_upload_form( $returnURL ) {
		if ( isset( $_REQUEST['paged'] ) ) {
			?>
		<input type="hidden" name="paged" value="<?php echo esc_attr( sanitize_text_field( $_REQUEST['paged'] ) ); ?>" />
			<?php
		}
		if ( isset( $_REQUEST['screen-options-apply'] ) ) {
			?>
		<input type="hidden" name="screen-options-apply" value="<?php echo esc_attr( sanitize_text_field( $_REQUEST['screen-options-apply'] ) ); ?>" />
			<?php
		}
		if ( isset( $_REQUEST['wp_screen_options']['option'] ) ) {
			?>
		<input type="hidden" name="wp_screen_options[option]" value="<?php echo esc_attr( sanitize_text_field( $_REQUEST['wp_screen_options']['option'] ) ); ?>" />
			<?php
		}
		if ( isset( $_REQUEST['wp_screen_options']['value'] ) ) {
			?>
		<input type="hidden" name="wp_screen_options[value]" value="<?php echo esc_attr( sanitize_text_field( $_REQUEST['wp_screen_options']['value'] ) ); ?>" />
			<?php
		}
		?>
	...
	<a href="<?php echo $returnURL; ?>" class="button-primary"><?php _e( 'Return', 'weblibrarian' ); ?></a></p>
						<?php
	}
	/* DB Maintenance page */
	...
	<div class="wrap"><div id="icon-weblib-db-maint" class="icon32"><br /></div><h2><?php _e( 'Database Maintenance', 'weblibrarian' ); ?></h2>
		...
		<table><thead><tr><th>
			<?php
			_e( 'Transaction', 'weblibrarian' );
			?>
				</th><th>
				<?php
				_e( 'Barcode', 'weblibrarian' );
				?>
				</th><th>
				<?php
				_e( 'Patron ID', 'weblibrarian' );
				?>
				</th><th>
				<?php
				_e( 'Title', 'weblibrarian' );
				?>
				</th><th>
				<?php
				_e( 'Date Out', 'weblibrarian' );
				?>
				</th><th>
				<?php
				_e( 'Due Data', 'weblibrarian' );
				?>
		...
		<tr><td>
						...
						<?php
									echo $ocheck->patronid();
						?>
		</td><td>
						<?php
									echo stripslashes( $ocheck->title() );
						?>
		</td><td>
						<?php
									echo mysql2date( 'M/j/Y', $ocheck->dateout() );
						?>
		</td><td>
						<?php
									echo mysql2date( 'M/j/Y', $ocheck->duedate() );
						?>
		</td></tr>
						<?php
				}
		...
		<table><thead><tr><th>
			...
			?>
				...
				<?php
				_e( 'Hold Date', 'weblibrarian' );
				?>
				</th><th>
				<?php
				_e( 'Expiration Date', 'weblibrarian' );
				?>
				</th></tr></thead><tfoot><tr><th>
				<?php
				_e( 'Transaction', 'weblibrarian' );
		...
		<tr><td>
						...
						<?php
									echo stripslashes( $ohold->title() );
						?>
		</td><td>
						<?php
									echo mysql2date( 'M/j/Y', $ohold->dateheld() );
						?>
		</td><td>
						<?php
									echo mysql2date( 'M/j/Y', $ohold->dateexpire() );
						?>
		</td></tr>
						<?php
				}
